...
const jBir = {
  ...
  // returns new image rotator object or undefined if selector returns no elements
  new: function(
    ...
    if ((newjBirImgRotator.elements.length = 0)) return; // selector string returned no elements; abort image rotator creation
    ...
    for (let i = 0; i < this.jBirImgRotators.length; i++) {
      ...
      if (
        ...
        continue; // leave loop since existing rotator was already found
      }
    }

    // Store original transition property
    newjBirImgRotator.og_transition_property =
      newjBirImgRotator.elements[0].style.transition;
    ...
    newjBirImgRotator.og_background_property =
      newjBirImgRotator.elements[0].style.background;
  ...
  next: function(jBirImgRotator) {
    ...
    if (
      ...
      indexOfNextImg = jBirImgRotator.image_url_arr.indexOf(
        this.jBirPreloadedImgs[0].src
      );
    } else {
      ...
      if (jBirImgRotator.random_order) {
        ...
        do {
          ...
          indexOfNextImg = Math.floor(
            Math.random() * (jBirImgRotator.image_url_arr.length - 1 - 0 + 1) +
              0
      ...
      // else randomize is off
        ...
        indexOfNextImg =
          jBirImgRotator.last_image + 1 >
          jBirImgRotator.image_url_arr.length - 1 // if we have reached the last image url
  ...
  remove: function(jBirImgRotator) {
    ...
    this.jBirImgRotators.splice(
      this.jBirImgRotators.indexOf(jBirImgRotator),
      1
    ); // Destroy image rotator
    return 0; // success
  },
...
class jBirImgRotator {
  ...
  image_url_arr = [
    ...
    "not an actual url so it will fail to preload and will be removed from the image url array"
  ];
